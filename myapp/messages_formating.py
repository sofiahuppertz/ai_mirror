...
def get_conversation_path(client, session, user_message):
    ...
    response = get_json_response(client, "gpt-3.5-turbo-0125", messages)
    print(response)
...
def get_most_similar_question(client, user_message, model, question_embeddings, df):
    ...
    messages = [{'role': 'system', 'content': context}, {'role': 'user', 'content': user_message}]
    print(messages)
    response = get_json_response(client, "gpt-4-0125-preview", messages) #gpt-4-0125-preview is better but it's not working 
    response = json.loads(response)
    print("Response from get_json_response: ")
    print(response)
    print("-" * 50)
